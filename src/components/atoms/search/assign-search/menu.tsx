...
import { AgencyMember, UserCollectionSegment } from '@/graphql/generated';
...
interface Props {
	...
	TeamMembersData?: UserCollectionSegment;
...
const SearchMenuBody = ({ setSearchMenu, setChoosenAssignUserId, setChoosenAssignUserName, TeamMembersData }: Props) => {
	const theme = useTheme();

	const choosenAssignHandler = (member: AgencyMember) => {
		setChoosenAssignUserId(member?.id as number);
		setChoosenAssignUserName(member?.fullName as string);
		setSearchMenu(false);
	};
	...
	return (
		...
		>
			...
			{TeamMembersData?.items?.map((member) => (
				<Stack key={member?.id} onClick={() => choosenAssignHandler(member as AgencyMember)}>
