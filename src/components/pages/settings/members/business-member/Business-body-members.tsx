...
import { BusinessMember, BusinessTypeMember, UserType, useUser_GetCurrentUserQuery } from '@/graphql/generated';
...
const BusinessMembersBody = ({ member }: Props) => {
	// -------------------------------tools
	...
	const editHandeler = () => {
		router.push({
			pathname: `/members/business-member-detail`,
			query: {
			...
			<StyledTableCell align="left">
				{member?.isDeleted ? (
					...
					<Stack direction={'row'} justifyContent={'start'} alignItems={'center'} spacing={3}>
						...
						CurrentUser?.businessAccesses?.[0]?.access?.isManageBusinessUserAccess === false ? null : (
							<Box sx={{ cursor: 'pointer' }} onClick={editHandeler}>
								<EditIcon fill={theme?.palette?.infuuse?.blue100} />
							</Box>
						)}

						{CurrentUser?.isBusinessOwner && CurrentUser?.user?.id !== member?.id ? (
						  ...
						  member?.typeMember !== BusinessTypeMember?.Owner &&
...
export default BusinessMembersBody;
