...
import {
	AccountStatus,
	AgencyAdminDetailsDto,
	useAgency_EditByAdminMutation,
...
const ActiveModal = ({ handleClose, agency }: Props) => {
	...
	const { mutate: UpdateBusiness } = useAgency_EditByAdminMutation();
			...
			},
			{
				onSuccess: (data) => {
					...
					if (status.code == 1) {
						...
						handleClose();
						querClient.invalidateQueries(['agency_getDetailsByAdmin']);
					} else {
