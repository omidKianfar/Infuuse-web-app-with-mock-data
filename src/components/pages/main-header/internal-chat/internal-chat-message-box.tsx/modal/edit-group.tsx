...
import { Conversation, ConversationType, useConversation_EditMutation, useConversationMember_AddListMutation} from '@/graphql/generated';
...
const EditGroup = ({ handleClose, conversation }: Props) => {
	const theme = useTheme();

	// -------------------------------states
	const [choosenGroupUser, setChoosenGroupUser] = useState([]);
	console.log('choosenGroupUser', choosenGroupUser);

	// -------------------------------query
	const { mutate: assignUsers } = useConversationMember_AddListMutation()
	const { mutate: editConversationTitle } = useConversation_EditMutation()
		...
		editConversationTitle(
			{
				...
				input: {
					...
					type: ConversationType?.InternalChat,
					contactId: null,
					isPinned: false
				}
			},
	...
	return (
		...
		>
			...
			<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
				...
				{/* ------------------------- user list */}
				<Stack height={'100%'} maxHeight={'200px'} overflow={'auto'} >
					<Stack spacing={2} mb={2}>
						<Typography fontWeight={'bold'} color={theme?.palette?.infuuse?.blue500}>New Member List</Typography>

						{choosenGroupUser?.length
							? choosenGroupUser?.map((user) => (
								<Stack
									key={user.id}
									direction={'row'}
							...
							: <Stack direction={'row'}
								justifyContent={'center'}
								alignItems={'center'}>
								<Typography fontWeight={'bold'} color={theme?.palette?.infuuse?.gray500}>Empty</Typography>
							</Stack>
						}
					</Stack>
					...
					<Stack spacing={2} mt={2}>
						<Typography fontWeight={'bold'} color={theme?.palette?.infuuse?.blue500}>Current Member List</Typography>

						{conversation?.conversationMembers?.map((user) => (
							<Stack
								key={user?.userId}
								direction={'row'}
								justifyContent={'space-between'}
							...
							>
								<Stack direction={'row'} justifyContent={'start'} alignItems={'center'}>
									<Avatar src={getFullImageUrl(user?.user?.photoUrl)} width={'32px'} height={'32px'} />

									<Typography color={theme?.palette?.infuuse?.blueDark500} fontWeight={'bold'} ml={1}>
										{stringSlicer(user?.user?.fullName, 40)}
									</Typography>
								</Stack>
							</Stack>
						))}
