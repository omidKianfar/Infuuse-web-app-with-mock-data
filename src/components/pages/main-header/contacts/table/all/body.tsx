...
import { Contact, DealStatus, useContact_GetByContactIdQuery } from '@/graphql/generated';
...
const Body = ({ contact }: Props) => {
	...
	return (
		<StyledTableRow onClick={(e) => e.stopPropagation()}>
			...
			<StyledTableCell align="left">
				<Typography fontSize={'14px'}>{contact?.business?.name}</Typography>
			</StyledTableCell>
			...
			<ModalContainer open={open} handleClose={handleClose}>
				<EditContactModal handleClose={handleClose} contact={contact} />
			</ModalContainer>
		</StyledTableRow>
	);
};
